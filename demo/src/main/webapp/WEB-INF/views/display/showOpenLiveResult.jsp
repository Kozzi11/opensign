...
<head>
    ...
    <title><fmt:message key="openlive.result.title"/></title>
</head>
<body bgcolor="white">
    ...
    <form name="form" method="post" action="${contextPath}/servlet/XMLDisplay">
        <input type="hidden" name="xml" value="${openLive.message}">
        ...
        <table border=1>
            ...
            <c:forEach var="prop" items="${signatureProperties}">
                <tr>
                    ...
                    <td>
                        ${prop.value.visibleToSigner}
                    </td>
                    <td>
                        <c:choose>
                            <c:when test="${prop.key eq 'signtext'}">
                                <textarea cols="80" rows="10" readonly>${prop.value.value}</textarea>
                            </c:when>
                            <c:otherwise>
                                ${prop.value.value}
                            </c:otherwise>
